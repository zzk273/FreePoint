...
def voxelize(
    ...
    if "labels" in input_dict:
        list_labels = input_dict["labels"] # 0:semantic, 1:instance, 2:segment
...
def get_instance_masks(
    ...
    for batch_id in range(len(list_labels)):
        ...
        for instance_id in instance_ids:
            ...
            # the semantic class of the instance
            label_id = tmp[0, 0]
        ...
        if task == "semantic_segmentation":
            ...
            if list_segments:
                ...
                target.append(
                    {
                        "labels": torch.zeros_like(label_ids, device=l.device),
                        "masks": masks,
                        "segment_mask": segment_masks,
                    }
                )
            else:
                target.append({"labels": torch.zeros_like(label_ids, device=l.device), "masks": masks})
        else:
            ...
            if list_segments:
                target.append(
                    {
                        "labels": torch.zeros_like(l, device=l.device),
                        "masks": masks,
                        "segment_mask": segment_masks,
                    }
                )
            else:
                target.append({"labels": torch.zeros_like(l, device=l.device), "masks": masks})
    return target
